...
**Four output formats per run** — markdown, HTML, JSON (default), PDF (opt-in) — written to `reports/YYYYMMDD-HHmmss/` and named `{domain-without-tld}-{timestamp}.{ext}` (e.g. `nvco-github-20260519-120212.html`). Each format is configured as a `FormatConfig` object with `enabled` and `showPagesAudited` booleans. `showPagesAudited` controls whether the audited URL list appears in that format's report. `showToolCredit` (default `false`) adds a "Generated by site-audit-cli" credit line to HTML and markdown reports.
