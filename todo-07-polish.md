...
- [ ] Full run: `node dist/index.js` against the test site — report generates cleanly
- [ ] Named config profile: `node dist/index.js config-examples/accessibility.json` — runs correctly
- [ ] PDF command: `node dist/index.js pdf reports/<base>` — PDFs generated cleanly
- [ ] Suppress list: add a known rule to `suppress` in config, confirm it disappears from report
- [ ] Review report output — issues are accurate, formatting is clean

## Done When
- [ ] No raw stack traces reach the user under any expected error condition
- [ ] `config.example.json` accurately reflects all available config options
- [ ] Local run confirmed working end-to-end
- [ ] Docker is deferred — final packaging step after all phases are complete
