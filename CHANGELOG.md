...
### Added
- `showToolCredit` config flag (default `false`) — adds "Generated by site-audit-cli (github.com/nvco/site-audit-cli)" credit line under the report title in HTML and markdown reports
  - Set to `true` in all three config profiles

### Fixed
- `Dockerfile` — changed `CMD` to `ENTRYPOINT` so config path arguments passed to `docker compose run` are correctly forwarded to `node dist/index.js` instead of replacing the command

### Changed
- README and CLAUDE.md updated to reflect new `FormatConfig` object shape, `showPagesAudited`, and `showToolCredit`
